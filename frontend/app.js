...
const MARKETPLACE = [
  ...
  { id: 'adguard', name: 'AdGuard Home', category: 'Networking', icon: '🛡️',
    description: 'Network-wide ad and tracker blocker with a polished web UI.',
    compose: `services:\n  adguardhome:\n    image: adguard/adguardhome:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "3000:3000"\n      - "53:53/tcp"\n      - "53:53/udp"\n    volumes:\n      - /opt/homelab/adguard/work:/opt/adguardhome/work\n      - /opt/homelab/adguard/conf:/opt/adguardhome/conf` },
  { id: 'audiobookshelf', name: 'Audiobookshelf', category: 'Media', icon: '🎧',
    description: 'Self-hosted audiobook and podcast server.',
    compose: `services:\n  audiobookshelf:\n    image: ghcr.io/advplyr/audiobookshelf:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "13378:80"\n    volumes:\n      - /opt/homelab/audiobookshelf/config:/config\n      - /opt/homelab/audiobookshelf/metadata:/metadata\n      - /media/audiobooks:/audiobooks\n      - /media/podcasts:/podcasts` },
  { id: 'calibre-web', name: 'Calibre-Web', category: 'Media', icon: '📚',
    description: 'Web UI for browsing and reading your Calibre ebook library.',
    compose: `services:\n  calibre-web:\n    image: lscr.io/linuxserver/calibre-web:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8083:8083"\n    volumes:\n      - /opt/homelab/calibre-web/config:/config\n      - /opt/homelab/calibre-web/books:/books\n    environment:\n      - PUID=1000\n      - PGID=1000` },
  { id: 'dashy', name: 'Dashy', category: 'Dashboard', icon: '🗂️',
    description: 'Highly customisable homelab start page and dashboard.',
    compose: `services:\n  dashy:\n    image: lissy93/dashy:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8090:8080"\n    volumes:\n      - /opt/homelab/dashy/config.yml:/app/user-data/conf.yml` },
  { id: 'filebrowser', name: 'File Browser', category: 'Tools', icon: '📁',
    description: 'Web-based file manager — browse, upload, delete files from your browser.',
    compose: `services:\n  filebrowser:\n    image: filebrowser/filebrowser:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8088:80"\n    volumes:\n      - /:/srv\n      - /opt/homelab/filebrowser/database.db:/database.db\n      - /opt/homelab/filebrowser/settings.json:/.filebrowser.json` },
  { id: 'freshrss', name: 'FreshRSS', category: 'Productivity', icon: '📰',
    description: 'Self-hosted RSS feed aggregator with a clean interface.',
    compose: `services:\n  freshrss:\n    image: lscr.io/linuxserver/freshrss:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8081:80"\n    volumes:\n      - /opt/homelab/freshrss/config:/config\n    environment:\n      - PUID=1000\n      - PGID=1000` },
  { id: 'home-assistant', name: 'Home Assistant', category: 'Smart Home', icon: '🏡',
    description: 'Open source home automation platform.',
    compose: `services:\n  homeassistant:\n    image: ghcr.io/home-assistant/home-assistant:stable\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    network_mode: host\n    volumes:\n      - /opt/homelab/homeassistant/config:/config\n    environment:\n      - TZ=UTC` },
  { id: 'mealie', name: 'Mealie', category: 'Productivity', icon: '🍽️',
    description: 'Self-hosted recipe manager and meal planner.',
    compose: `services:\n  mealie:\n    image: ghcr.io/mealie-recipes/mealie:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "9925:9000"\n    volumes:\n      - /opt/homelab/mealie:/app/data\n    environment:\n      - ALLOW_SIGNUP=true\n      - TZ=UTC` },
  { id: 'navidrome', name: 'Navidrome', category: 'Media', icon: '🎵',
    description: 'Modern music server and streamer compatible with Subsonic clients.',
    compose: `services:\n  navidrome:\n    image: deluan/navidrome:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "4533:4533"\n    volumes:\n      - /opt/homelab/navidrome/data:/data\n      - /media/music:/music:ro\n    environment:\n      - ND_SCANSCHEDULE=1h\n      - ND_LOGLEVEL=info` },
  { id: 'netdata', name: 'Netdata', category: 'Monitoring', icon: '📡',
    description: 'Real-time performance and health monitoring for servers.',
    compose: `services:\n  netdata:\n    image: netdata/netdata:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "19999:19999"\n    cap_add:\n      - SYS_PTRACE\n    volumes:\n      - /proc:/host/proc:ro\n      - /sys:/host/sys:ro\n      - /var/run/docker.sock:/var/run/docker.sock:ro` },
  { id: 'qbittorrent', name: 'qBittorrent', category: 'Downloads', icon: '⬇️',
    description: 'Feature-rich torrent client with a web UI.',
    compose: `services:\n  qbittorrent:\n    image: lscr.io/linuxserver/qbittorrent:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8080:8080"\n      - "6881:6881"\n      - "6881:6881/udp"\n    volumes:\n      - /opt/homelab/qbittorrent/config:/config\n      - /media/downloads:/downloads\n    environment:\n      - PUID=1000\n      - PGID=1000\n      - WEBUI_PORT=8080` },
  { id: 'radarr', name: 'Radarr', category: 'Downloads', icon: '🎥',
    description: 'Automated movie downloader and library manager.',
    compose: `services:\n  radarr:\n    image: lscr.io/linuxserver/radarr:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "7878:7878"\n    volumes:\n      - /opt/homelab/radarr/config:/config\n      - /media/movies:/movies\n      - /media/downloads:/downloads\n    environment:\n      - PUID=1000\n      - PGID=1000` },
  { id: 'sonarr', name: 'Sonarr', category: 'Downloads', icon: '📺',
    description: 'Automated TV show downloader and library manager.',
    compose: `services:\n  sonarr:\n    image: lscr.io/linuxserver/sonarr:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8989:8989"\n    volumes:\n      - /opt/homelab/sonarr/config:/config\n      - /media/tv:/tv\n      - /media/downloads:/downloads\n    environment:\n      - PUID=1000\n      - PGID=1000` },
  { id: 'stirling-pdf', name: 'Stirling PDF', category: 'Tools', icon: '📝',
    description: 'Locally hosted web app for PDF manipulation — merge, split, convert.',
    compose: `services:\n  stirling-pdf:\n    image: frooodle/s-pdf:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8080:8080"\n    volumes:\n      - /opt/homelab/stirling-pdf/training-data:/usr/share/tessdata\n      - /opt/homelab/stirling-pdf/configs:/configs` },
  { id: 'syncthing', name: 'Syncthing', category: 'Tools', icon: '🔄',
    description: 'Continuous file synchronisation between devices.',
    compose: `services:\n  syncthing:\n    image: lscr.io/linuxserver/syncthing:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8384:8384"\n      - "22000:22000/tcp"\n      - "22000:22000/udp"\n    volumes:\n      - /opt/homelab/syncthing/config:/config\n      - /opt/homelab/syncthing/data:/data\n    environment:\n      - PUID=1000\n      - PGID=1000` },
  { id: 'trilium', name: 'Trilium Notes', category: 'Productivity', icon: '🗒️',
    description: 'Hierarchical note-taking app with rich text, code, and relations.',
    compose: `services:\n  trilium:\n    image: zadam/trilium:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8070:8080"\n    volumes:\n      - /opt/homelab/trilium:/home/node/trilium-data` },
  { id: 'wg-easy', name: 'WireGuard Easy', category: 'Networking', icon: '🔒',
    description: 'The easiest way to run WireGuard VPN with a web UI.',
    compose: `services:\n  wg-easy:\n    image: ghcr.io/wg-easy/wg-easy:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "51820:51820/udp"\n      - "51821:51821/tcp"\n    cap_add:\n      - NET_ADMIN\n      - SYS_MODULE\n    sysctls:\n      - net.ipv4.ip_forward=1\n      - net.ipv4.conf.all.src_valid_mark=1\n    volumes:\n      - /opt/homelab/wireguard:/etc/wireguard\n    environment:\n      - WG_HOST=your-domain-or-ip\n      - PASSWORD=changeme123` },
  { id: 'firefly', name: 'Firefly III', category: 'Finance', icon: '💰',
    description: 'Self-hosted personal finance manager — track expenses, budgets, bills.',
    compose: `services:\n  firefly:\n    image: fireflyiii/core:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8086:8080"\n    volumes:\n      - /opt/homelab/firefly/upload:/var/www/html/storage/upload\n    environment:\n      - APP_KEY=changeme32characterslongkey123\n      - DB_CONNECTION=sqlite\n      - TRUSTED_PROXIES=**` },
  { id: 'kavita', name: 'Kavita', category: 'Media', icon: '📖',
    description: 'Fast, feature-rich digital library for manga, comics, and books.',
    compose: `services:\n  kavita:\n    image: jvmilazz0/kavita:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "5000:5000"\n    volumes:\n      - /opt/homelab/kavita/config:/kavita/config\n      - /media/books:/books` },
  { id: 'monica', name: 'Monica', category: 'Productivity', icon: '👥',
    description: 'Personal CRM — keep notes on friends, family, and contacts.',
    compose: `services:\n  monica:\n    image: monica:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8087:80"\n    volumes:\n      - /opt/homelab/monica:/var/www/html/storage\n    environment:\n      - APP_KEY=changeme32characterslongkey123\n      - DB_CONNECTION=sqlite` },
  { id: 'komga', name: 'Komga', category: 'Media', icon: '📗',
    description: 'Comics and manga server with a web reader and OPDS support.',
    compose: `services:\n  komga:\n    image: gotson/komga:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "25600:25600"\n    volumes:\n      - /opt/homelab/komga/config:/config\n      - /media/comics:/data\n    environment:\n      - TZ=UTC` },
  { id: 'homepage', name: 'Homepage', category: 'Dashboard', icon: '🏠',
    description: 'Modern, highly customisable application dashboard with service integrations.',
    compose: `services:\n  homepage:\n    image: ghcr.io/gethomepage/homepage:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "3000:3000"\n    volumes:\n      - /opt/homelab/homepage/config:/app/config\n      - /var/run/docker.sock:/var/run/docker.sock:ro` },
  { id: 'seafile', name: 'Seafile', category: 'Productivity', icon: '🌊',
    description: 'High-performance file sync and sharing platform.',
    compose: `services:\n  seafile:\n    image: seafileltd/seafile-mc:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8084:80"\n    volumes:\n      - /opt/homelab/seafile:/shared\n    environment:\n      - SEAFILE_ADMIN_EMAIL=admin@example.com\n      - SEAFILE_ADMIN_PASSWORD=changeme123\n      - SEAFILE_SERVER_HOSTNAME=localhost` },
  { id: 'jellyseerr', name: 'Jellyseerr', category: 'Media', icon: '🎟️',
    description: 'Media request manager for Jellyfin, Radarr and Sonarr.',
    compose: `services:\n  jellyseerr:\n    image: fallenbagel/jellyseerr:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "5055:5055"\n    volumes:\n      - /opt/homelab/jellyseerr/config:/app/config\n    environment:\n      - TZ=UTC` },
  { id: 'prowlarr', name: 'Prowlarr', category: 'Downloads', icon: '🔎',
    description: 'Indexer manager and proxy for Radarr, Sonarr, and other *arr apps.',
    compose: `services:\n  prowlarr:\n    image: lscr.io/linuxserver/prowlarr:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "9696:9696"\n    volumes:\n      - /opt/homelab/prowlarr/config:/config\n    environment:\n      - PUID=1000\n      - PGID=1000\n      - TZ=UTC` },
  { id: 'authelia', name: 'Authelia', category: 'Security', icon: '🔑',
    description: 'Single sign-on and two-factor authentication portal for your apps.',
    compose: `services:\n  authelia:\n    image: authelia/authelia:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "9091:9091"\n    volumes:\n      - /opt/homelab/authelia:/config\n    environment:\n      - TZ=UTC` },
  { id: 'kaizoku', name: 'Kaizoku', category: 'Media', icon: '🏴‍☠️',
    description: 'Self-hosted manga downloader that keeps your library up to date.',
    compose: `services:\n  kaizoku:\n    image: ghcr.io/oae/kaizoku:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8000:8000"\n    volumes:\n      - /opt/homelab/kaizoku/data:/data\n      - /media/manga:/manga\n    environment:\n      - TZ=UTC` },
  { id: 'authentik', name: 'Authentik', category: 'Security', icon: '🛂',
    description: 'Flexible identity provider and SSO platform.',
    compose: `services:\n  authentik:\n    image: ghcr.io/goauthentik/server:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    command: server\n    ports:\n      - "9000:9000"\n      - "9443:9443"\n    volumes:\n      - /opt/homelab/authentik/media:/media\n      - /opt/homelab/authentik/templates:/templates\n    environment:\n      - AUTHENTIK_SECRET_KEY=changeme32characterslongkey12345\n      - AUTHENTIK_REDIS__HOST=redis\n      - AUTHENTIK_POSTGRESQL__HOST=postgresql\n      - AUTHENTIK_POSTGRESQL__USER=authentik\n      - AUTHENTIK_POSTGRESQL__PASSWORD=changeme\n      - AUTHENTIK_POSTGRESQL__NAME=authentik` },
  { id: 'pterodactyl', name: 'Pterodactyl Panel', category: 'Management', icon: '🦖',
    description: 'Game server management panel with a beautiful web UI.',
    compose: `services:\n  panel:\n    image: ghcr.io/pterodactyl/panel:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8091:80"\n    volumes:\n      - /opt/homelab/pterodactyl/var:/app/var\n      - /opt/homelab/pterodactyl/nginx:/etc/nginx/http.d\n      - /opt/homelab/pterodactyl/logs:/app/storage/logs\n    environment:\n      - APP_URL=http://localhost:8091\n      - APP_TIMEZONE=UTC\n      - APP_SERVICE_AUTHOR=admin@example.com\n      - DB_PASSWORD=changeme\n      - HASHIDS_SALT=changeme` },
  { id: 'matrix', name: 'Matrix Synapse', category: 'Communication', icon: '💬',
    description: 'Decentralised, encrypted messaging and collaboration server.',
    compose: `services:\n  synapse:\n    image: matrixdotorg/synapse:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8448:8448"\n    volumes:\n      - /opt/homelab/matrix/data:/data\n    environment:\n      - SYNAPSE_SERVER_NAME=localhost\n      - SYNAPSE_REPORT_STATS=no` },
  { id: 'romm', name: 'RomM', category: 'Gaming', icon: '🕹️',
    description: 'Beautiful ROM manager — scan, enrich, and play your game library.',
    compose: `services:\n  romm:\n    image: rommapp/romm:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8998:8080"\n    volumes:\n      - /opt/homelab/romm/resources:/romm/resources\n      - /media/roms:/romm/library\n      - /opt/homelab/romm/assets:/romm/assets\n      - /opt/homelab/romm/config:/romm/config\n    environment:\n      - DB_PASSWD=changeme\n      - ROMM_AUTH_SECRET_KEY=changeme32characterslongkey12345` },
  { id: 'lazylibrarian', name: 'Lazy Librarian', category: 'Downloads', icon: '📕',
    description: 'Automated book, magazine and audiobook downloader.',
    compose: `services:\n  lazylibrarian:\n    image: lscr.io/linuxserver/lazylibrarian:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "5299:5299"\n    volumes:\n      - /opt/homelab/lazylibrarian/config:/config\n      - /media/downloads:/downloads\n      - /media/books:/books\n    environment:\n      - PUID=1000\n      - PGID=1000\n      - TZ=UTC` },
  { id: 'kiwix', name: 'Kiwix', category: 'Tools', icon: '🌐',
    description: 'Offline web content server — read Wikipedia, Project Gutenberg and more without internet.',
    compose: `services:\n  kiwix:\n    image: ghcr.io/kiwix/kiwix-serve:latest\n    container_name: {{STACK_NAME}}\n    restart: unless-stopped\n    ports:\n      - "8888:8080"\n    volumes:\n      - /opt/homelab/kiwix/data:/data\n    command: /data/*.zim` },
];
